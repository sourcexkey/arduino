...
#define TOP_RSSI_PIN        2
#define PAN_SERVO_PIN       5     // Pan servo pin
#define TOP_SERVO_PIN       6     // Pan servo pin
...
uint16_t rssi_top_array[FIR_SIZE];
...
void mainLoop() {
  ...
  uint16_t avgTop = max(avg(rssi_top_array, FIR_SIZE) + RSSI_OFFSET_TOP, RSSI_MIN);
  uint16_t angBottom = (float(avgRight + avgLeft) / 2)
...
//  }
  ...
  */
  ...
  uint8_t dynamicDeadbandTop = (float(angBottom + avgTop) / 2 - RSSI_MIN) / (RSSI_MAX - RSSI_MIN) * DEADBAND;

  // if target is in the middle, don't move
  if (abs(avgRight - avgLeft) > dynamicDeadband ) {
    movePanByLeftRight(avgRight, avgLeft);
  } 
  if (abs(avgTop - angBottom) > dynamicDeadbandTop ) {
    movePanUpDown(avgTop, angBottom);
  } 
}

void movePanByLeftRight(uint16_t avgLeft, uint16_t avgRight) {
   float ang = 0;
...
void movePanUpDown(uint16_t avgTop, uint16_t avgBottom) {
   float ang = 0;
  ...
  #if defined(EXPONENTIAL)
    float x = float(avgTop - avgBottom);
    x = x * x / 500;
    ang = x * SERVO_DIRECTION * -1;
  #endif

  #if defined(RELATIVE)
    ang = float(avgTop / avgBottom) * (SERVO_DIRECTION * -1);
  #endif

  #if defined(SIGMOID)
    float x = float(avgTop - avgBottom) / 10;
    x = SERVO_MAX_STEP / (1+ exp(-SIGMOID_SLOPE * x + SIGMOID_OFFSET));
    ang = x * SERVO_DIRECTION * -1;
  #endif
  ...
  #if defined(EXPONENTIAL)
    float x = float(avgBottom - avgTop);
    x = x * x / 500;
    ang = x * SERVO_DIRECTION;
  #endif

  #if defined(RELATIVE)
    ang = float(avgBottom / avgTop) * SERVO_DIRECTION;
  #endif

  #if defined(SIGMOID)
    float x = float(avgBottom - avgTop) / 10;
    x = SERVO_MAX_STEP / (1+ exp(-SIGMOID_SLOPE * x + SIGMOID_OFFSET));
    ang = x * SERVO_DIRECTION;
  #endif
...
//    Serial.print(map(avgRight, RSSI_MIN, RSSI_MAX, 0, 100));
    ...
    // raw rssi values, use these for RSSI_MIN and RSSI_MAX
    Serial.print("Calibration - bottom: ");
...
void moveTopBy(float angle) {

  anglePan += angle;
  anglePan = limit(SERVO_MIN, SERVO_MAX, anglePan);
  servoTop.write(anglePan);
}
